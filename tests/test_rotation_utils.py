...
def test_angles_from_imu_fusion_without_magneto():
...
def test_angles_from_imu_fusion_with_magneto():
    """Test angles_from_imu_fusion function with magnetometer data."""
    np.random.seed(42)
    ...
    magnetometer = np.zeros((3, n_frames))
    magnetometer[0, :] = np.linspace(-10, -100, n_frames)
    magnetometer[1, :] = np.linspace(0, -50, n_frames)
